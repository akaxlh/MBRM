...
class Recommender:
	...
	def ours(self):
		...
		uPred = self.predict(ulats, self.uids, self.origin_iids, 'item')
		iPred = self.predict(ilats, self.iids, self.origin_uids, 'user')
		pred = uPred + iPred
		return pred
	...
	def prepareModel(self):
		...
		for i in range(args.intTypes):
			self.uVecs.append(tf.sparse_placeholder(tf.float32, name='uvec'+str(i)))
		...
		self.origin_uids = tf.placeholder(name='or_uids', dtype=tf.int32, shape=[None])
		self.origin_iids = self.iids
	...
	def trainEpoch(self):
		...
		for i in range(steps):
			...
			uMap = dict([(batIds[i], i) for i in range(len(batIds))])
			new_uLocs = list(map(lambda x: uMap[x], uLocs))
			batItms = list(set(iLocs))
			feeddict = {self.uids: new_uLocs, self.iids: iLocs, self.origin_uids: uLocs}
			for j in range(args.intTypes):
				tem = self.trnMats[j][batIds]
				feeddict[self.uVecs[j]] = transToLsts(tem)
	...
	def testEpoch(self):
		...
		ids = self.tstUsrs
		num = len(ids)
		testbatch = np.maximum(32, args.batch * args.sampNum * 2 // 100)
		...
		for i in range(steps):
			...
			feeddict = {self.uids: new_uLocs, self.iids: iLocs, self.origin_uids: uLocs}
			for j in range(args.intTypes):
				feeddict[self.uVecs[j]] = transToLsts(self.trnMats[j][batIds])
